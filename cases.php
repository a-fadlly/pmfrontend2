...
if (empty($_SESSION["access_token"])) {
  ...
  $inbox = filterArray($cases->cases->data, ['usr_uid' => $_SESSION['usr_uid'], 'app_status' => 'TO_DO']);
  $draft = filterArray($cases->cases->data, ['app_status' => 'DRAFT']);
          ...
          <div class="card card-custom">
            <div class="tab-content">
                      ...
                      <tbody>
                        ...
                        if (!empty($inbox)) {
                          ...
                          foreach ($inbox as $key => $d) {
                            ...
                            } else if ($d->app_tas_title == "Accounting Review" && $d->app_pro_title == "Purchase Request V.02") {
                              $url = "AccountingReview.php?app_uid=" . $d->app_uid;
                            } else if ($d->app_tas_title == "Director of Production Approval" && $d->app_pro_title == "Purchase Request V.02") {
                              $url = "ApprovalProductionDirector.php?app_uid=" . $d->app_uid;
                            } else if ($d->app_tas_title == "Review by HR/GA Manager" && $d->app_pro_title == "Purchase Request V.02") {
                              $url = "ApprovalHRGAManager.php?app_uid=" . $d->app_uid;
                            } else if ($d->app_tas_title == "Accounting HO Analysis" && $d->app_pro_title == "Purchase Request V.02") {
                              $url = "AccountingHOAnalysis.php?app_uid=" . $d->app_uid;
                            } else if ($d->app_tas_title == "CFAO Approval" && $d->app_pro_title == "Purchase Request V.02") {
                              $url = "ApprovalCFAO.php?app_uid=" . $d->app_uid;
                            } else if ($d->app_tas_title == "President Director/Vice President Approval" && $d->app_pro_title == "Purchase Request V.02") {
                              $url = "ApprovalPresidentDirector.php?app_uid=" . $d->app_uid;
                            } else {
                            ...
                            <tr>
                              ...
                              <td><?= $d->usrcr_usr_firstname ?> <?= $d->usrcr_usr_lastname ?></td>
              ...
              <div class="tab-pane fade" id="kt_tab_pane_4_4" role="tabpanel" aria-labelledby="kt_tab_pane_4_4">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                  <div class="card-title">
                    <h3 class="card-label">Paused
                      <span class="d-block text-muted pt-2 font-size-sm">A case can be paused, meaning that it is temporarily unavailable and cannot be opened or worked on</span>
                    </h3>
                  </div>
                </div>
                <div class="card-body">
